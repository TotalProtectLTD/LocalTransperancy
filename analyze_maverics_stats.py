...
def analyze_metric(metric_name: str, old_data: dict, new_data: dict) -> Dict:
    ...
    if changed_points:
        changes = [abs(new_val - old_val) for _, old_val, new_val in changed_points]
...
def print_analysis_report(old_file: str, new_file: str):
    ...
    for result in all_results:
        ...
        if len(result['changed_points']) > 0:
            change_pct = (len(result['changed_points']) / result['overlap_length']) * 100
            print(f"  - Change percentage: {change_pct:.1f}%")
            print(f"  - Average change magnitude: {result['avg_change']:.1f}")
            print(f"  - Max change: {result['max_change']}")
            print(f"  - Min change: {result['min_change']}")
        
        if result['changed_points']:
            print(f"\n  Sample of changed data points (showing first 10):")
...
if __name__ == "__main__":
    old_file = "/Users/rostoni/Downloads/maverics.txt"
    new_file = "/Users/rostoni/Downloads/maverics2.txt"
